...
import { useRegionMetrics } from "../hooks/useMetrics";
...
function MetricsList({ region }) {
  const { topServers, totalServers, totalJoins, loading, refetch } = useRegionMetrics(region);
...
export default function MetricsPage() {
  return (
    <div className="min-h-screen px-2 py-12 pt-24" style={{
      background: "radial-gradient(ellipse 70% 40% at 50% 0%, rgba(0,229,255,0.07) 0%, transparent 60%), #0a0a0f",
    }}>
      ...
      <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row gap-6">
        <div className="flex-1"><MetricsList region="EU" /></div>
        <div className="flex-1"><MetricsList region="US" /></div>
      </div>
    </div>
  );
}